...
@Component({
  selector: 'app-library-exchange-archived',
  templateUrl: './library-exchange-archived.component.html',
  styleUrls: ['./library-exchange-archived.component.scss']
})
export class LibraryExchangeArchivedComponent implements OnInit {
  userId: string
  ...
  archiveConfirmation(CopyRequestVM: LibraryExchangeRequestVM){
    ...
    this.confirmationModalService.confirmAction = this.service.UnArchivedAndUpdateGiftBookRequest;
  ...
  GetArchivedCopyRequests() {
    this.spinner.show();
    this.service.GetArchivedLibraryExchangeRequests(this.userId).then(
      (res) => {
