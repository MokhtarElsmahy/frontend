...
export class ProfileComponent implements OnInit {
  ...
  DeactivatestaticAlertClosed = true;
  @ViewChild('staticAlert', {static: false}) staticAlert: NgbAlert;
  @ViewChild('staticAlertDeactivate', {static: false}) staticAlertDeactivate: NgbAlert;
  userData: FormData = new FormData();
  FilesUrl = environment.files_url
  ...
  ConfirmDeactivate(){
    ...
    this.openModal();
    this.confirmationModalService.modalType = 'Deactivate';
    this.confirmationModalService.message =  (this.translate.currentLang == 'en' ? "Are you sure, you want to remove account ?"
                                                                                                   : "هل أنت متأكد من رغبتك فى الغاء الحساب ؟")
  ...
  Deactivate(){
    this.spinner.show();
    this.userService.Deactivate(this.userId).then((res) => {
      ...
      if(result==true){
        ...
        this.spinner.hide();
        this.DeactivatestaticAlertClosed = false;
        setTimeout(() => {
          this.staticAlertDeactivate.close();
        }, 10000);
      }
