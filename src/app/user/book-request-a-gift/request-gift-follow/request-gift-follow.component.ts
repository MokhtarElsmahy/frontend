...
@Component({
  selector: 'app-request-gift-follow',
  templateUrl: './request-gift-follow.component.html',
  styleUrls: ['./request-gift-follow.component.scss']
})
export class RequestGiftFollowComponent  implements OnInit{
  decesion = true;
  userId: string
      ...
      .finally(() => {
        if(!this.global.Messages.find(m => m.type == MessageEnum.Error)){
          ...
          setTimeout(() => {
            ...
            this.router.navigateByUrl("/auth/libraryServices/request-gift-view");
          }, 2000);
        }     
      });
  ...
  getServiceRatingByUserId() {
    this.serviceRatingService.getServiceRatingByUserId(this.userId, 8).then((res) => {
