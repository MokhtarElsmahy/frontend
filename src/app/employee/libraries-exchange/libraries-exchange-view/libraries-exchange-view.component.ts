...
export class LibrariesExchangeViewComponent {
  userId: string
  ...
  GetAllRequests() {
    this.spinner.show();
    this.libraryService.getAllLibraryExchangeRequests(this.userId).then(
      (res) => {
  ...
  archiveConfirmation(CopyRequestVM: LibraryExchangeRequestVM){
    ...
    this.confirmationModalService.confirmAction = this.libraryService.addToArchivedAndUpdateGiftBookRequest;
